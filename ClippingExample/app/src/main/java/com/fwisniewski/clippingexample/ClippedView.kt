package com.fwisniewski.clippingexample
...
class ClippedView @JvmOverloads constructor(
    ...
    private fun drawClippedRectangle(canvas: Canvas) {
        ...
        paint.textAlign = Paint.Align.RIGHT // Align the RIGHT side of the text with the origin.
    ...
    private fun drawDifferenceClippingExample(canvas: Canvas) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O)
            canvas.clipRect(
                4 * rectInset, 4 * rectInset, clipRectRight - 4 * rectInset,
    ...
    private fun drawIntersectionClippingExample(canvas: Canvas) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            canvas.clipRect(
                ...
                clipRectRight, clipRectBottom, Region.Op.INTERSECT
            )
    ...
    private fun drawCombinedClippingExample(canvas: Canvas) {
        ...
        path.rewind()
        path.addCircle(
            clipRectLeft + rectInset + circleRadius, clipRectTop + circleRadius + rectInset,
            circleRadius, Path.Direction.CCW
        )

        path.addRect(
            clipRectRight / 2 - circleRadius, clipRectTop + circleRadius + rectInset,
            clipRectRight / 2 + circleRadius, clipRectBottom - rectInset, Path.Direction.CCW
        )
    ...
    private fun drawQuickRejectExample(canvas: Canvas) {
        val inClipRectangle = RectF(
            clipRectRight / 2, clipRectBottom / 2,
            clipRectRight * 2, clipRectBottom * 2
        )

        val notInClipRectangle = RectF(
            RectF(
                clipRectRight + 1, clipRectBottom + 1,
                clipRectRight * 2, clipRectBottom * 2
            )
        )
